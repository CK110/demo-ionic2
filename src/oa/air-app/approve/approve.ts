...
import {AIR_APP_APPROVE_BATCH_INIT, AIR_APP_APPROVE_SINGLE_INIT} from "../../../api/api";
import {BatchApprovePage} from "../../../pages/common/batch-approve/batch-approve";

@IonicPage()
@Component({
  selector: 'air-app-approve',
  templateUrl: 'approve.html',
})
export class AirAppApprovePage {
  activePage:number = 0;
  private pList:any[]=[];
  ...
  async init(param:any){
    ...
    await this.httpClient.post(AIR_APP_APPROVE_BATCH_INIT,param).map(res=>res.json()).subscribe((res)=>{
      m.detail = res['detail'];
  ...
  async getDetail(param:any){ //不查询current
    let detail:any;
    await this.httpClient.post(AIR_APP_APPROVE_SINGLE_INIT,param).map(res=>res.json()).subscribe((res)=>{
      detail = res
    })
    return detail;
  }
